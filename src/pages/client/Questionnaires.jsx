...
export default function ClientQuestionnaires() {
  ...
  const reload = async () => {
    const list = await questionnaireApi.listByClient(user.clientId)
  ...
  const handleCreate = async () => {
    if (!formTitle.trim()) { alert('Informe o título do questionário'); return }
    setSaving(true)
    try {
      const q = await questionnaireApi.create({ clientId: user.clientId, title: formTitle.trim(), description: formDesc.trim() || null })
  ...
  const handleDelete = async (q) => {
    if (!confirm(`Excluir "${q.title}"? Todas as perguntas serão removidas.`)) return
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.5)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center' }}>
          <div style={{ background:'#fff', borderRadius:16, padding:32, maxWidth:480, width:'90%', boxShadow:'0 20px 60px rgba(0,0,0,.2)' }}>
            ...
            <div style={{ display:'flex', gap:8, marginTop:20 }}>
              ...
              <Button variant="primary" full disabled={saving} onClick={handleCreate}>{saving ? '⏳...' : 'Criar'}</Button>
            </div>
          </div>
        </div>
      )}
