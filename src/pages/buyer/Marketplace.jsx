...
export default function BuyerMarketplace() {
  ...
  const renderStage = () => {
    if (stage === 1) return <div><div style={{ fontFamily:'Montserrat,sans-serif',fontWeight:700,fontSize:16,color:'#1a1c5e',marginBottom:16 }}>Qual categoria de fornecedor você procura?</div><div style={{ display:'flex',flexWrap:'wrap',gap:10,marginBottom:20 }}>{CATEGORIES.map(c=><button key={c} onClick={()=>upd('category',c)} style={chipStyle(filters.category===c)}>{c}</button>)}</div><input value={filters.q} onChange={e=>upd('q',e.target.value)} placeholder="Busca livre por nome ou serviço..." style={inputStyle} /></div>
    ...
    if (stage === 6) return <div><div style={{ fontFamily:'Montserrat,sans-serif',fontWeight:700,fontSize:16,color:'#1a1c5e',marginBottom:16 }}>Confirme os filtros</div><div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:10 }}>{[['Categoria',filters.category||'Todas'],['Região',filters.state],['Porte',filters.size],['Nível',filters.level],['Busca',filters.q||'—']].map(([k,v])=><div key={k} style={{ padding:'12px 14px',background:'rgba(46,49,146,.04)',borderRadius:10,border:'1px solid rgba(46,49,146,.08)' }}><div style={{ fontSize:10,color:'#9B9B9B',fontFamily:'Montserrat,sans-serif',textTransform:'uppercase',letterSpacing:.5 }}>{k}</div><div style={{ fontSize:14,fontFamily:'DM Sans,sans-serif',fontWeight:600,color:'#1a1c5e',marginTop:2 }}>{v}</div></div>)}</div></div>
  }
