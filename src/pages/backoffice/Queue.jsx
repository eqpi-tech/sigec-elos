      ...
      {filtered.length===0?<EmptyState icon="✅" title="Fila vazia" subtitle="Nenhum item para o filtro selecionado"/>:filtered.map((s,i)=>(
        <Card key={i} hover style={{ borderRadius:14,padding:'16px 20px',marginBottom:12 }}>
          <div style={{ display:'flex',alignItems:'center',gap:16 }}>
            ...
            <div style={{ flex:1 }}>
              <div style={{ display:'flex',alignItems:'center',gap:10 }}><div style={{ fontSize:15,fontWeight:700,color:'#1a1c5e',fontFamily:'Montserrat,sans-serif' }}>{s.razaoSocial}</div><span style={{ fontSize:10,fontWeight:700,color:RISK_COLOR[s.riskLevel],background:`${RISK_COLOR[s.riskLevel]}18`,padding:'2px 10px',borderRadius:20,fontFamily:'Montserrat,sans-serif' }}>Risco {s.riskLevel}</span></div>
              <div style={{ fontSize:12,color:'#9B9B9B',marginTop:2 }}>{s.cnpj} · {s.requestedAt}</div>
              <div style={{ display:'flex',gap:12,marginTop:6,alignItems:'center' }}><div style={{ width:160 }}><ScoreBar score={s.score||0}/></div><span style={{ fontSize:11,color:'#9B9B9B' }}>{s.documents?.filter(d=>d.status==='VALID').length||0}/{s.documents?.length||0} docs válidos</span></div>
            </div>
...
export function BackofficeAnalysis() {
  ...
  useEffect(() => { adminApi.getSealAnalysis(id).then(d=>{setData(d);setLevel(d.seals?.[0]?.level||'Simples')}).finally(()=>setLoading(false)) },[id])

  const handleApprove = async () => { setProcessing(true); await adminApi.approveSeal(id,level); setDone('approved'); setProcessing(false) }
  const handleReject  = async () => { if(!obs){alert('Informe o motivo');return}; setProcessing(true); await adminApi.rejectSeal(id,obs); setDone('rejected'); setProcessing(false) }
  ...
  if (done) return (
    <div style={{ maxWidth:600,margin:'80px auto',padding:24,textAlign:'center' }}>
      ...
      <div style={{ fontFamily:'Montserrat,sans-serif',fontWeight:800,fontSize:22,color:'#1a1c5e',marginBottom:8 }}>{done==='approved'?`Selo ELOS ${level} emitido!`:'Solicitação rejeitada'}</div>
      <div style={{ fontFamily:'DM Sans,sans-serif',fontSize:15,color:'#9B9B9B',marginBottom:24 }}>{done==='approved'?'Fornecedor agora visível no marketplace.':`Motivo: "${obs}"`}</div>
        ...
        <div>
          <Card style={{ borderRadius:16,padding:'20px 24px',marginBottom:16 }}>
            <div style={{ display:'flex',gap:16,alignItems:'center',marginBottom:14 }}>
              <div style={{ width:56,height:56,borderRadius:14,background:'rgba(46,49,146,.1)',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:800,fontSize:22,color:'#2E3192',fontFamily:'Montserrat,sans-serif' }}>{data.razao_social?.[0]}</div>
              <div><div style={{ fontSize:18,fontWeight:800,color:'#1a1c5e',fontFamily:'Montserrat,sans-serif' }}>{data.razao_social}</div><div style={{ fontSize:13,color:'#9B9B9B' }}>{data.cnpj} · {data.city}/{data.state}</div></div>
            </div>
          ...
          <Card style={{ borderRadius:16,padding:'20px 24px' }}>
            <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16 }}>
              ...
              <div style={{ display:'flex',gap:8,fontSize:12 }}><span style={{ color:'#22c55e' }}>✓ {ok} ok</span>{miss>0&&<span style={{ color:'#ef4444' }}>✕ {miss} pendente</span>}</div>
            </div>
            {docs.map((doc,i)=>{
              ...
              return (
                <div key={i} style={{ display:'flex',alignItems:'center',gap:12,padding:'12px 14px',borderRadius:12,marginBottom:8,background:colors[status]||'#f4f5f9',border:`1px solid ${borders[status]||'#e2e4ef'}` }}>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ fontSize:11,color:'#9B9B9B' }}>{doc.source==='AUTO'?'Auto-coletado':'Upload manual'}{doc.expires_at?` · vence ${doc.expires_at.slice(0,10)}`:''}</div>
                  </div>
                  {doc.storage_path&&<Button variant="neutral" size="sm" onClick={async()=>{const url=await documentApi.getSignedUrl(doc.storage_path);window.open(url,'_blank')}}>👁 Ver</Button>}
                  {status==='PENDING'&&<>
                    {actn==='loading'?<Spinner size={16}/>:<>
                      <Button variant="success" size="sm" onClick={()=>handleDocAction(doc.id,'VALID')}>✓ Aprovar</Button>
                      <Button variant="danger" size="sm" onClick={()=>handleDocAction(doc.id,'REJECTED')}>✕</Button>
                    </>}
                  </>}
                </div>
              )
        ...
        <div style={{ position:'sticky',top:80,alignSelf:'flex-start' }}>
          <Card style={{ borderRadius:16,padding:'20px 24px',border:'2px solid #e2e4ef' }}>
            ...
            <div style={{ padding:'12px',background:miss>0?'rgba(239,68,68,.08)':'rgba(34,197,94,.08)',borderRadius:10,border:`1px solid ${miss>0?'rgba(239,68,68,.2)':'rgba(34,197,94,.2)'}`,marginBottom:16,textAlign:'center' }}>
              <div style={{ fontFamily:'Montserrat,sans-serif',fontWeight:700,fontSize:13,color:miss>0?'#dc2626':'#16a34a' }}>{miss>0?`⚠ ${miss} doc(s) pendente(s)`:'✅ Documentação completa'}</div>
            ...
            <div style={{ marginBottom:14 }}>
              ...
              <div style={{ display:'flex',gap:8 }}>
                {['Simples','Premium'].map(l=><button key={l} onClick={()=>setLevel(l)} style={{ flex:1,padding:'10px',borderRadius:10,border:`2px solid ${level===l?'#2E3192':'#e2e4ef'}`,background:level===l?'rgba(46,49,146,.08)':'#fff',color:level===l?'#2E3192':'#9B9B9B',fontFamily:'Montserrat,sans-serif',fontWeight:700,fontSize:13,cursor:'pointer' }}>{l==='Premium'?'⭐':'🏷️'} {l}</button>)}
              </div>
            </div>
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:12,fontFamily:'Montserrat,sans-serif',fontWeight:600,color:'#1a1c5e',marginBottom:6 }}>Observação</div>
              <textarea value={obs} onChange={e=>setObs(e.target.value)} placeholder="Observações ou motivo de rejeição..." rows={3} style={{ width:'100%',padding:'10px 12px',borderRadius:10,border:'1px solid #e2e4ef',fontFamily:'DM Sans,sans-serif',fontSize:13,color:'#1a1c5e',resize:'vertical',boxSizing:'border-box' }}/>
            </div>
            <div style={{ display:'flex',flexDirection:'column',gap:8 }}>
              <Button variant="success" full size="lg" style={{ borderRadius:10 }} disabled={processing} onClick={handleApprove}>{processing?'⏳...':'✅ Aprovar Selo '+level}</Button>
              <Button variant="danger" full size="md" style={{ borderRadius:10 }} disabled={processing} onClick={handleReject}>❌ Rejeitar</Button>
