-- patch_075_list_documents_split.sql (18/09)
-- Análise de Docs: FILA (limite de análise) e STATUS viram filtros
-- INDEPENDENTES — p_queue (todos|fila|passados|hoje|futuros) combina por AND
-- com p_status (status reais do documento). Substitui a assinatura anterior.

DROP FUNCTION IF EXISTS public.admin_list_documents(text, text, date, text, text, int, int);

CREATE OR REPLACE FUNCTION public.admin_list_documents(
  p_doc_type text DEFAULT NULL, p_status text DEFAULT 'todos',
  p_queue text DEFAULT 'todos',
...
BEGIN
  ...
  base AS (
    ...
    WHERE d.label IS NOT NULL
      ...
      -- filtro FILA (limite de análise — regra HOC)
      AND CASE coalesce(p_queue,'todos')
            ...
            ELSE true
          END
      -- filtro STATUS (status reais do documento)
...
REVOKE ALL ON FUNCTION public.admin_list_documents(text, text, text, date, text, text, int, int) FROM public;
GRANT EXECUTE ON FUNCTION public.admin_list_documents(text, text, text, date, text, text, int, int) TO authenticated;
